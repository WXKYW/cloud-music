...
- 🎨 **统一卡片样式**：歌单和电台卡片样式与歌手卡片保持一致（圆形封面、hover overlay效果）
- 🎵 **歌词同步优化**：将歌词提前补偿从0.3秒增加到0.5秒，提升歌词与歌曲的同步精度
- 🎵 **API 源重构**：移除 Vercel 依赖，引入 I-Meto、Cyril Studio 等优质公共源，稳定性大幅提升
- 🔄 **智能播放回退**：版权受限时提供更清晰的跳过提示和自动重试机制 (P0-2)
- 📱 **移动端触控升级**：智能识别水平/垂直滑动，解决播放器区域误触问题 (P1-1)
...
- **全新多源架构**：整合 I-Meto (Meting)、Cyril Studio (NCM)、Wuenci 等优质公共 API 源
- **智能源切换**：版权受限或请求失败时，自动尝试备用源，确保高可用性
- **动态优先级**：根据源的响应速度和成功率自动优化请求顺序
...
- 👆 上滑：滚动内容
- 👇 下滑：滚动内容
- ✨ **智能防误触**：在播放器/歌词区域优先响应垂直滚动，仅在明确横向滑动时切换页面
...
- 🔁 请求重试：指数退避算法，提高成功率
- 📊 请求合并：批量操作合并请求
- 🛡️ **版权回退**：基于历史数据的智能跳过机制 (Playback Analytics)
...
### v3.5.1 (2025-11-30) - 样式统一与歌词优化版 🎨

#### 🎨 UI 样式统一
- **卡片样式统一**：歌单和电台卡片与歌手卡片保持一致
  - 封面改为圆形（`border-radius: 50%`）
  - hover时显示渐变overlay和播放图标
  - overlay中显示名称

#### 🎵 歌词同步优化
- **提前补偿增加**：从0.3秒增加到0.5秒
- **同步精度提升**：歌词显示更准确地与歌曲同步

### v3.5.0 (2025-11-30) - API 重构与体验升级版 🎵

#### 🎵 API 源全面升级
- **多源架构**：不再依赖单一平台，引入 I-Meto、Cyril Studio 等优质公共源
- **格式识别**：精准识别 NCM 和 Meting 格式，兼容更多第三方 API
- **稳定性**：大幅提升海外和弱网环境下的可用性

#### 🚀 播放体验优化 (P0-2)
- **智能跳过**：根据历史播放记录，自动跳过频繁失败的音源
- **清晰反馈**：当触发跳过或降级时，提供明确的用户提示
- **精准统计**：详细记录分音质的播放失败情况

#### 📱 移动端触控升级 (P1-1)
- **智能手势**：彻底解决垂直滚动与水平切页的冲突
- **区域感知**：在歌词、列表等可滚动区域，优先响应滚动操作
- **流畅度**：保持切页动画流畅的同时，消除误触烦恼
...
- [I-Meto API](https://api.i-meto.com) - 优质稳定的 Meting API
- [Cyril Studio](https://music.cyrilstudio.top) - 优秀的 NCM API 服务
- [Wuenci API](https://api.wuenci.com) - 稳定的 API 支持
